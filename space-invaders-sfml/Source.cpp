//----------------------------------------------------SpaceInvadersGame Developed by Khansa 242887, Amna 242881, Nayab 242864--------------------------------------
...
protected:
    ...
    float lastBombTime = 0.f;  
    float bombCooldown = 5.f; 


    void setupTexture(const std::string& filepath, float scaleSize, float ) {
...
public:
    ...
    virtual float getBombSpeed() const { return 180.f; } 
    ...
    virtual bool isBombReady(float currentTime) const {
        return aligned && (currentTime - lastBombTime) >= bombCooldown;
    }

    virtual void restartBombTimer(float currentTime) {
...
public:
    ...
    {
        setupTexture("assets/alpha_invader.png", 50.f, 5.f); 
        bombCooldown = 5.f;
...
private:
    ...
    float diveDelay = static_cast<float>(rand() % 3 + 1); 
...
public:
    ...
    {
        setupTexture("assets/monster.png", 180.f, 0);  
        ...
        lightningSprite.setScale(0.1f, 0.7f);  
    }
    ...
    sf::FloatRect getBeamBounds() const {
        return lightningSprite.getGlobalBounds();  
    }
...
public:
    Dragon() {
        if (!texture.loadFromFile("assets/dragon2.png")) {
            std::cerr << "[ERROR] Failed to load dragon.png\n";
        }

        sprite.setTexture(texture);
        sprite.setScale(0.4f, 0.4f);
        sprite.setPosition(240.f, 20.f);

        fireBeam.setSize(sf::Vector2f(6.f, 600.f));
        fireBeam.setFillColor(sf::Color(255, 100, 0, 180));
        fireBeam.setOrigin(3.f, 0.f);
    }

    void activate(sf::Vector2f) {
        active = true;
        alreadyDodged = false;
        isFiring = false;
    ...
    void update(float dt) {
        ...
        if (!isFiring && beamClock.getElapsedTime().asSeconds() >= beamCooldown) {
            int dir = rand() % 3;

            if (dir == 0) {
                fireDirection = { 0.f, 1.f };
                fireBeam.setRotation(0.f);
            }
            else if (dir == 1) {
                fireDirection = sf::Vector2f(-1.0f, 1.f); 
                fireBeam.setRotation(-55.f);
            }
            else {
                fireDirection = sf::Vector2f(1.0f, 1.f);  
                fireBeam.setRotation(55.f);
            ...
            sf::Vector2f base = sprite.getPosition() + sf::Vector2f(sprite.getGlobalBounds().width / 2.f, sprite.getGlobalBounds().height);
            fireBeam.setPosition(base);
            isFiring = true;
            beamClock.restart();
        }

        if (isFiring && beamClock.getElapsedTime().asSeconds() >= beamDuration) {
            isFiring = false;
            beamClock.restart();
        }
    ...
    void draw(sf::RenderWindow& window) {
        if (!active) return;
        window.draw(sprite);
        if (isFiring) window.draw(fireBeam);
    }

    bool isActive() const { return active; }

    bool checkCollision(sf::FloatRect playerBounds) {
        return active && isFiring && fireBeam.getGlobalBounds().intersects(playerBounds);
    }

    bool hasDodged() const { return alreadyDodged; }

    void markDodged() { alreadyDodged = true; }
};
...
public:
    MenuScreen() {
        ...
        backgroundTexture.loadFromFile("assets/mainscreen.jpeg");
...
public:
    InstructionScreen() {
        font.loadFromFile("assets/impact.ttf");
        try {
            if (!backgroundTexture.loadFromFile("assets/b2.png")) {

               throw runtime_error("Failed to load b2.png");
            }
        }
        catch (const exception& e) {
            cout << "Texture Load Error: " << e.what() << endl;
        }
        backgroundSprite.setTexture(backgroundTexture);
        backgroundSprite.setScale(
            800.f / backgroundTexture.getSize().x,
...
public:
    PauseScreen() {
        ...
        backgroundTexture.loadFromFile("assets/pauseScreen.jpg");
        ...
        pauseTitle.setFillColor(sf::Color(140,131, 145));
        pauseTitle.setOutlineColor(sf::Color::Black);
        pauseTitle.setOutlineThickness(3);
        pauseTitle.setPosition(230.f, 160.f);
        ...
        resumeText.setPosition(230.f, 260.f);

        quitText.setFont(font);
        quitText.setString("Quit");
        quitText.setCharacterSize(26);
        quitText.setFillColor(sf::Color(189, 206, 217));
        quitText.setOutlineColor(sf::Color::Black);
        quitText.setPosition(250.f, 310.f);
...
public:
    GameOverScreen() {
        ...
        backgroundTexture.loadFromFile("assets/gameOver.jpg");
        ...
        gameOverText.setPosition(220, 170);
        ...
        backText.setFillColor(sf::Color(192, 192, 192));
        backText.setOutlineColor(sf::Color::Black);
        backText.setOutlineThickness(1);
        backText.setStyle(sf::Text::Italic);
        backText.setPosition(250, 360);

        completionMessage.setString("");  
        completionMessage.setFont(font);
        completionMessage.setCharacterSize(22);
        completionMessage.setFillColor(sf::Color(192, 192, 192));
        completionMessage.setOutlineColor(sf::Color::Black);
        completionMessage.setOutlineThickness(1.5f);
        completionMessage.setPosition(230, 250); 
    ...
    void setCompletionMessage(const std::string& msg) {
        completionMessage.setString(msg);
    }
...
public:
    NameInputScreen() {
        font.loadFromFile("assets/Orbitron-Regular.ttf");
        backgroundTexture.loadFromFile("assets/nameInput.jpg");
        backgroundSprite.setScale(
            800.f / backgroundTexture.getSize().x,
            600.f / backgroundTexture.getSize().y
        );
        ...
        underline.setFillColor(sf::Color(189, 191, 170));
        ...
        promptText.setFillColor(sf::Color(159, 181, 194));
        promptText.setStyle(sf::Text::Bold);
        promptText.setOutlineColor(sf::Color::Black);
        promptText.setOutlineThickness(2);
        ;
        subtitle.setFont(font);
        subtitle.setString("Press ENTER to start mission");
        subtitle.setCharacterSize(16);
        subtitle.setFillColor(sf::Color(159, 181, 194));
        subtitle.setPosition(200, 350);


        nameText.setFont(font);
        nameText.setCharacterSize(24);
        nameText.setFillColor(sf::Color(189, 191, 170));
        ...
        infoText.setFillColor(sf::Color(189, 191, 170));
        ...
        warningText.setString("Enter name to proceed!");
        warningText.setPosition(200.f, 300.f); 
...
public:
    HighScoreScreen(HighScoreManager& mgr) : manager(mgr) {
        ...
        if (!backgroundTexture.loadFromFile("assets/highscore.jpg")) {
            std::cerr << "[ERROR] Could not load highscore.png\n";
        }
        backgroundSprite.setTexture(backgroundTexture);
        backgroundSprite.setScale(
            800.f / backgroundTexture.getSize().x,
...
private:
    ...
    float monsterTriggerTime = 10.f + rand() % 10; 
    bool showMonsterWarning = false;

    sf::Clock globalBombClock;
    float globalBombInterval = 1.5f; 
    ...
    Dragon dragon;
    sf::Clock dragonSpawnClock;
    bool dragonEligible = false;
    sf::Clock dragonWarningClock;
    bool showDragonWarning = false;
    bool dragonAppeared = false;
    sf::Texture backgroundTexture;
    sf::Sprite backgroundSprite;
    sf::Text dragonWarningText;
...
public:
    Game() : window(sf::VideoMode(800, 600), "Space Invaders"), score(0) {
        ...
        if (!backgroundTexture.loadFromFile("assets/mainscreen.jpeg")) {
            std::cerr << "[ERROR] Could not load mainscreen.jpeg                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            qq                                  \n";
        }
        backgroundSprite.setTexture(backgroundTexture);
        backgroundSprite.setScale(
            800.f / backgroundTexture.getSize().x,
        ...
        dragonWarningText.setFont(font);
        dragonWarningText.setCharacterSize(28);
        dragonWarningText.setFillColor(sf::Color::Red);
        dragonWarningText.setString("!!! DRAGON INCOMING !!!");
        dragonWarningText.setPosition(200.f, 270.f);
    ...
    void fireBullets() {
        ...
        if (player.isPoweredUp) {
            ...
            for (int i = 0; i < numBullets; ++i) {
                ...
                sf::Vector2f dir(sin(angleRad), -cos(angleRad));
    ...
    void update() {
        ...
        if (levelManager.getLevel() == 3 && levelManager.getWave() > 1) {
            dragonEligible = true;
        }

        if (dragonEligible && !dragonAppeared && !dragon.isActive() && !showDragonWarning && dragonSpawnClock.getElapsedTime().asSeconds() > 15.f) {
            showDragonWarning = true;
            dragonWarningClock.restart();
        }

        if (showDragonWarning && dragonWarningClock.getElapsedTime().asSeconds() >= 2.f) {
            dragon.activate(player.getPosition());
            dragonAppeared = true;
            showDragonWarning = false;
            dragonSpawnClock.restart();
        ...
        if (dragon.isActive() && dragon.checkCollision(player.getBounds()) && !player.isPoweredUp) {
            player.lives--;
            if (player.lives <= 0) {
        ...
        if (!dragon.isActive() && !dragon.hasDodged()) {
            score += 50;
            dragon.markDodged();
        }

        if (!monsterHasAppeared && !monsterActive && !showMonsterWarning && levelManager.getLevel() == 1 && levelManager.getWave() != 1 &&
        ...
        if (!monsterActive && !dragon.isActive()) {
            for (auto* e : invaders)
                e->update(dt);
            ...
            if (globalBombClock.getElapsedTime().asSeconds() >= globalBombInterval) {
                ...
                float absoluteTime = monsterTriggerClock.getElapsedTime().asSeconds();  

                std::vector<Invader*> readyInvaders;
                for (auto* e : invaders) {
                    if (e->isBombReady(absoluteTime)) {
                        readyInvaders.push_back(e);
                    }
                }

                std::random_shuffle(readyInvaders.begin(), readyInvaders.end());
                ...
                for (int i = 0; i < std::min(maxDrops, static_cast<int>(readyInvaders.size())); ++i) {
                    ...
                    readyInvaders[i]->restartBombTimer(absoluteTime);
                }
            }
        ...
        if (!monsterActive && !dragon.isActive()) {
            ...
            if (invaders.empty()) {
                // Check if Level 3 Wave 3 just ended
                if (levelManager.getLevel() == 3 && levelManager.getWave() == 3) {
                    gameOverScreen.setFinalScore(score);
                    gameOverScreen.setCompletionMessage("ALL LEVELS COMPLETED!");
                    highScoreManager.addNewScore(playerName, score);
                    currentState = GameState::GameOver;
                    return;
                }
    ...
    void render() {
        ...
        if (showDragonWarning) {
            window.draw(dragonWarningText);
        }
        dragon.draw(window);
        window.draw(scoreText);
